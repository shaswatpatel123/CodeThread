...
from .parser_constants import MAP_REPO_TO_PARSER
...
def get_eval_report(
    ...
    if f2p_ref is None or f2p_ref == "":
        raise Exception("Excepted FAIL_TO_PASS to exists")

    p2p_ref = test_spec[PASS_TO_PASS]
    if f2p_ref is None or f2p_ref == "":
        raise Exception("Excepted PASS_TO_PASS to exists")


    # Convert into list[str]. Not the case in original swe-bench as they are have custom dataclass TestSpec
    f2p_ref = json.loads( f2p_ref )
    p2p_ref = json.loads( p2p_ref )
