...
@dataclass
class MaintainabilityFastEnvironmentConfig:
    image: str
    cwd: str = "/"
...
class MaintainabilityFastEnvironment:
    def __init__(
        self, *, config_class: type = MaintainabilityFastEnvironmentConfig, logger: logging.Logger | None = None, **kwargs
    ):

        """Maintainability fast environment. See `MaintainabilityFastEnvironmentConfig` for kwargs."""

        self.logger = logger or logging.getLogger("minisweagent.environment")
        self.config = config_class(**kwargs)
        self.sandbox_dir = Path(tempfile.gettempdir()) / f"minisweagent-{uuid.uuid4().hex[:8]}"

        self.files_to_change = kwargs.get( 'files_to_change', None )
        self.init_patch = kwargs.get( 'init_patch', None )
        ...
        self.git_checkout("maintainBench")

    def git_commit(self, commit_message: str):
        self.execute("git add .", timeout=180)
        self.execute(f"git commit --no-verify -m '{commit_message}'")

    def git_checkout(self, branch_name: str):
    ...
    def reset_changes(self):
        self.execute("git reset --hard HEAD", timeout=240)
        self.execute("git clean -fd", timeout=240)

    def move_files_to_container(self):
        if self.files_to_change is not None:
            for file_to_change in self.files_to_change:
                ...
                self.logger.info(f"{target_path} replaced with {host_file}")
        else:
            raise Exception("No files_to_change")

    def apply_init_patch(self):
        if self.init_patch is None:
            raise ValueError("init_patch is None in MaintainabilityFastEnvironment")
        self.apply_patch(self.init_patch)

    def apply_patch(self, patch_file: Path):
        patch_file_container = f"{self.config.cwd}/patch.diff"
        print("copy_to_container in apply_patch")
        ...
        for filepath in filtered_files:
            try:
                ...
                if language == "python":
                    ...
                    # lizard-based detection for non-Python
                    try:
    ...
    def get_maintainability_metrics(self, instance_id: str, language: str = "python") -> dict:
        ...
        # Get modified files (relative paths)
        val = self.execute("git ls-files --others --modified --exclude-standard", timeout=600)
        if val['returncode'] != 0:
            self.logger.error("Failed to get modified files")
            return result
        ...
        self.logger.info(f"[DEBUG] instance={instance_id} language={language} git_ls_files_raw={modified_files} filtered={filtered_files}")
        if not filtered_files:
            self.logger.warning(f"[DEBUG] No files after filtering for {instance_id}. Raw git output: {val['output']!r}")

        for idx, filepath in enumerate(filtered_files):
            try:
                # Check if file is newly added
                old_version_result = self.execute(f"git show HEAD:{filepath}", timeout=600)
                ...
                # Get new version from working tree
                new_version_result = self.execute(f"cat {filepath}", timeout=600)
                if new_version_result['returncode'] != 0:
                    result[filepath] = [{
                        "info": "ERROR",
                        "reason": f"Failed to read file: {filepath}"
    ...
    def execute(self, command: str, cwd: str = "", timeout: int | None = None) -> dict[str, Any]:
        ...
        if self.multilingual:
            bind = f"{str(self.sandbox_dir)}/root:/root"
            cmd.extend(["--bind", bind])
            bind = f"{str(self.sandbox_dir)}/tmp:/tmp"
            cmd.extend(["--bind", bind])
    ...
    def reset_repo(self):
        _res = self.execute(f"cd {self.config.cwd} && rm -rf .git")
        if _res['returncode'] != 0:
            raise Exception(f"Removing .git file failed, {_res}")
        _, _, _, _returncode = self.exec_run_with_timeout(f"cd {self.config.cwd} && git init && git config user.email 'maintainCoder@email.com' && git config user.name 'maintainCoder' && git add . && git commit -m 'Clean benchmark snapshot' --no-verify", timeout=240)
        if _returncode != 0:
