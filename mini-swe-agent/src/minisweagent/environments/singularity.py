...
class SingularityEnvironmentConfig:
    ...
    skip_git_reset: bool = False
    """Skip .git removal — needed for run-only-eval so entryscripts can git checkout test files."""
...
class SingularityEnvironment:
    def __init__(
        ...
        if files_to_change is not None or init_patch is not None:
            ...
            self.execute("git config user.email 'maintainCoder@email.com'")
            self.execute("git config user.name 'maintainCoder'")
            ...
            if _res['returncode'] != 0 and "nothing to commit" not in _res['output']:
        ...
        if not self.config.skip_git_reset:
            self.reset_repo()

    def reset_repo(self):
        _stdout, _stderr, _, _returncode = self.exec_run_with_timeout(f"cd {self.config.cwd} && rm -rf .git", timeout=600)
        if _returncode != 0:
            raise Exception(f"Removing .git file failed, {_stderr}")
        _stdout, _stderr, _, _returncode = self.exec_run_with_timeout(f"cd {self.config.cwd} && git init && git config user.email 'maintainCoder@email.com' && git config user.name 'maintainCoder' && git add . && git commit --allow-empty -m 'Clean benchmark snapshot' --no-verify", timeout=600)
        if _returncode != 0:
            raise Exception(f"Reset the repo failed: stdout={_stdout}, stderr={_stderr}")
    ...
    def _setup_sandbox(self, files_to_change: list[str] | None = None, init_patch: list[Path] | None = None):
        ...
        if files_to_change is not None:
            print("files_to_change:", files_to_change)
    ...
    def get_maintainability_metrics(self, instance_id: str, language: str = "python") -> dict:
        ...
        # Get modified files (relative paths)
        val = self.execute("git ls-files --others --modified --exclude-standard", timeout=1200)
        if val['returncode'] != 0:
            self.logger.error("Failed to get modified files")
            return result
    ...
    def cleanup(self):
        if self.sandbox_dir.exists():
            ...
            try:
                ...
                quarantine_base = Path(os.getenv("QUARANTINE_BASE", "/scratch/spp9399/tmp_extra"))
                dest = quarantine_base / f"{root.name}-{int(time.time())}-{uuid.uuid4().hex[:8]}"
                shutil.move(str(root), str(dest))
            except Exception:
                self.logger.warning("Failed to move or delete: %s", self.sandbox_dir)
