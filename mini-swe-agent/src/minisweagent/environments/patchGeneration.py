...
@dataclass
class PatchGenerationEnvironmentConfig:
    image: str
    cwd: str = "/"
...
class PatchGenerationEnvironment:
    def __init__(
        self, *, config_class: type = PatchGenerationEnvironmentConfig, logger: logging.Logger | None = None, **kwargs
    ):

        """Patch generation environment. See `PatchGenerationEnvironmentConfig` for kwargs."""
        ...
        #     self.logger.info( ">>>>>>MOVING CORRUPTION FILES")
        #     self.move_corruption_files(files_to_change)
        ...
        #     self.git_commit("Changes for Patch Generation")
    ...
    def move_corruption_files(self):
    ...
    def git_commit(self, commit_message: str):
        ...
        self.execute(f"git commit -m '{commit_message}'")

    def apply_patch(self, patch_file: Path | None = None):
        if patch_file is None:
            patch_file = self.init_patch
        if not patch_file.exists():
            patch_file = Path( str(patch_file).replace("patch_generated.diff", "patch.diff") )
            if not patch_file.exists():
        ...
        GIT_APPLY_CMDS = [
                ...
                "git apply --whitespace=nowarn --verbose --reject",
                "patch --batch --fuzz=5 -p1 -i",
        ]

        for git_apply_cmd in GIT_APPLY_CMDS:
            result = self.execute(f"{git_apply_cmd} /testbed/patchxyz.diff")
            ...
            self.logger.info(f"Failed to apply patch to container: {git_apply_cmd}, error: {result['output']}")
