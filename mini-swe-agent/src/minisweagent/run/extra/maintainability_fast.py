...
def main(
    ...
    with Live(progress_manager.render_group, refresh_per_second=4):
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(process_instance, instance, output_path, config, progress_manager, run_only_eval, path_local_images, oracle_first_pr, gold_patch_path, swebench_patch_path, first_pr_patch_path, second_pr_patch_path): instance[
                    "instance_id"
                ]
                for instance in instances
            }
