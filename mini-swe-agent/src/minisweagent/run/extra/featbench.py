...
from minisweagent.evaluation.featbench import evaluation, report
...
DATASET_MAPPING = {
    ...
    "featbench": "/scratch/spp9399/MaintainableCoder/minisweagent/experiments/data/featbench/featbench_v1_0.json",
...
def build_remote_name(instance: dict, path_local_images: Path | None) -> str:
    repo = instance.get("repo")
    number = instance.get("number")
    if repo and number:
        image_short = "ghcr.io_kndy666_" f"featbench_{repo.replace('/', '_').lower()}_{number}.sif"

    return image_short

def get_sb_environment(config: dict, instance: dict, path_local_images: Path | None, multilingual, skip_git_reset: bool = False) -> Environment:
    image_name = build_remote_name(instance, path_local_images)
    ...
    env_config['init_patch'] = None
    runtime_config = {}
    runtime_config['need_gpu'] = True
    if multilingual:
        env_config['multilingual'] = True

    env_config['runtime_config'] = runtime_config
    if skip_git_reset:
...
def process_instance(
    ...
    try:
        ...
        # Featbench clones the repo in env.pull_image() below, so the in-__init__ reset_repo
        # would run against an empty cwd. The explicit env.reset_repo() after pull_image handles it.
        env = get_sb_environment(config, instance, path_local_images, multilingual, skip_git_reset=True)

        env.pull_image( instance['repo'], instance['base_commit'] )

        if instance.get("init_patch", None) is not None:
            for i_patch in instance['init_patch']:
                env.apply_patch(i_patch)
        ...
        if len( result.strip() ) > 0 and not skip_eval:
        ...
        if skip_eval:
            exit_status = exit_status if exit_status != "None" else "Submitted"
...
def preprocess_instances(instances: list[dict]) -> list[dict]:
    ...
    def _build_complete_diff(patch: dict) -> str:
    ...
    for instance in instances:
        ...
        for patch in instance['test_patch']:
            test_patch.append( _build_complete_diff(patch) )

        test_patch = "\n".join(test_patch)

        instance['test_patch'] = test_patch

        # Convert FAIL_TO_PASS and PASS_TO_PASS to list of patches
        fail_to_pass = instance.get('FAIL_TO_PASS', None)
        pass_to_pass = instance.get('PASS_TO_PASS', None)
        if fail_to_pass is not None:
            # Split on ','
            fail_to_pass = fail_to_pass.split(',')
            instance['FAIL_TO_PASS'] = fail_to_pass
        if pass_to_pass is not None:
            # Split on ','
            pass_to_pass = pass_to_pass.split(',')
...
def handle_gold_test(instances: list[dict], output_path: Path) -> None:
    ...
    for instance in instances:
        ...
        for patch in instance['patch']:
            gold_patch.append( _build_complete_diff(patch) )
...
def main(
    ...
    skip_eval: bool = typer.Option(False, "--skip-eval", help="Run multilingual evaluation", rich_help_panel="Multilingual evaluation"),
) -> None:
    # fmt: on
    ...
    # list(load_dataset(dataset_path, split=split))

    if user_custom:
        instances = merge_instances(instances, instances_custom)


    instances = preprocess_instances(instances)
    ...
    with Live(progress_manager.render_group, refresh_per_second=4):
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(process_instance, instance, output_path, config, progress_manager, run_only_eval, path_local_images, task_column_name, multilingual, skip_eval): instance[
                    "instance_id"
                ]
                for instance in instances
            }       
