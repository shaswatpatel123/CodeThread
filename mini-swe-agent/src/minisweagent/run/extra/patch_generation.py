...
def process_instance(
    ...
    old_patch_file = Path(instance_dir  / "patch.diff")
    logger.info(f"OLD_PATCH_FILE: {old_patch_file}")

    instance_dir_real = Path(output_real) / instance_id
    second_pr_maintainability_metrics_file = Path(instance_dir_real / "second_pr_maintainability_metrics.json")
    ...
    try:       
        if not second_pr:
            ...
            progress_manager.update_instance_status(instance_id, "Generating patch")

            # Get current branch name
            current_branch = env.execute("git branch --show-current")['output'].strip()

            logger.info(f"CURRENT BRANCH: {current_branch}")

            if current_branch == "":
                # Use below to debug. The head is detached so need to create a new working branch
                # cmds = ["git rev-parse --show-toplevel","git status -sb","git rev-parse --abbrev-ref HEAD"]
                # for cmd in cmds:
                #     logger.info(f"{cmd}: {env.execute(cmd)['output'].strip()}")

                # Create a branch
                current_branch = "new_working_branch_xyz"
                env.execute(f"git checkout -b {current_branch}")

                assert env.execute("git branch --show-current")['output'].strip() == current_branch


            progress_manager.update_instance_status(instance_id, "Moving corruption files")
            
            # Branch_1 <-- corruption files (PR_0)
            env.execute("git checkout -b branch_1")
            env.move_corruption_files()
            env.git_commit("Changes for Patch Generation branch_1")

            # Branch_2 <-- init patch (PR_1)
            env.execute(f"git checkout {current_branch}")
            env.execute("git checkout -b branch_2")
            env.apply_patch()
            env.git_commit("Changes for Patch Generation branch_2")

            # Take the diff between branch_1 and branch_2
            patch = env.execute("git diff branch_1..branch_2")['output']
            ...
            # Branch_1 <-- corruption files (PR_0) + init patch (PR_1)
            env.execute("git checkout -b branch_1")
            env.move_corruption_files()
            progress_manager.update_instance_status(instance_id, "Applying patch to branch 1")
            env.apply_patch()
            env.git_commit("Changes for Patch Generation branch_1")

            # Branch_2 <-- second patch (PR_2)
            progress_manager.update_instance_status(instance_id, "Applying old_patch to branch 2")
            env.execute(f"git checkout {current_branch}")
            env.execute("git checkout -b branch_2")
            env.apply_patch(old_patch_file)
            env.git_commit("Changes for Patch Generation branch_2")

            # Removing any *patch.diff* occurrence before taking patch
            env.execute("rm -rf *patch.diff*")
            ...
            env.execute(f"git checkout {current_branch}")
            env.execute("git checkout -b maintainMetric")
            env.move_corruption_files()
            progress_manager.update_instance_status(instance_id, "Applying patch PR1 to maintainMetric")
            env.apply_patch()
            logger.info("Appying patch2 to maintainMetric")
...
def main(
    ...
    output_real: str = typer.Option("", "-o", "--output-real", help="Output directory", rich_help_panel="Basic"),
    oracle_first_pr: bool = typer.Option( False, "--oracle_first_pr", help="Oracle 1st PR", rich_help_panel="Sanity run"),
    second_pr: bool = typer.Option( False, "--second-pr", help="Second PR", rich_help_panel="Sanity run")
) -> None:
    # fmt: on
    ...
    logger.info(f"Is second-pr set: {second_pr}")
    ...
    if gold:
        ...
        print("FINDING astropy__astropy-12907:", write_dict['astropy__astropy-12907'])
    ######
    ...
    progress_manager = RunBatchProgressManager(len(instances), output_path / f"exit_statuses_patch_generation_{time.time()}.yaml")
    ...
    with Live(progress_manager.render_group, refresh_per_second=4):
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(process_instance, instance, output_path, config, progress_manager, run_only_eval, path_local_images, oracle_first_pr, second_pr, output_real): instance[
                    "instance_id"
                ]
                for instance in instances
            }
