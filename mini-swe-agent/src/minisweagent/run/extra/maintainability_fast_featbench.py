...
def get_sb_environment(config: dict, instance: dict, path_local_images: Path | None, multilingual) -> Environment:
...
def process_instance(
    ...
    try:
        ...
        init_patches = instance.get("init_patch", [])
        if not isinstance(init_patches, list):
            init_patches = [init_patches]
        env.apply_patch(Path(init_patches[0]))
        ...
        # Now apply PR_1
        print( "first_pr_patch:",  first_pr_patch )
...
def main(
    ...
    with Live(progress_manager.render_group, refresh_per_second=4):
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(process_instance, instance, output_path, config, progress_manager, run_only_eval, path_local_images, oracle_first_pr, gold_patch_path, swebench_patch_path, first_pr_patch_path, second_pr_patch_path, multilingual, gold): instance[
                    "instance_id"
                ]
                for instance in instances
            }
